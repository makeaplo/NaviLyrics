...
> 状态：生效中；2026-09-21 增量实现与验证见第 12 节。第 3–5 节保留 2026-08-18 的历史审计快照，第 6 节“事实”描述原始问题。
...
| LIB-001 | P1 | 待验证 | 分页已实现，自动化通过后仍需真实服务验收 |
| DAT-001 | P1 | 待验证 | 账号隔离已实现，迁移策略见第 12 节 |
| REC-001 | P1 | 待验证 | 已限制为 4 个并发请求，真实性能待测 |
...
**2026-09-21 进展**：实现 `offset` 分页、ID 去重、重复页失败、读取进度和取消；刷新仅发布完整结果，失败保留旧目录。测试包含 301、600、1,000 张专辑。
...
**2026-09-21 进展**：新增 `LibraryIdentity`，统一服务器规范化并使用服务器与用户名的 SHA-256 摘要作为账号标识；历史、行为、推荐、收藏与恢复队列全部使用该标识。旧数据保留但不自动认领。
...
**2026-09-21 进展**：改为滚动调度、最多 4 个并发请求，并对专辑和歌曲 ID 去重；保持返回顺序、取消和失败传播。加载与错误状态保留已有推荐。请求总数仍为唯一专辑数，未宣称已消除 N+1。
...
- 2026-09-21：实现专辑分页、账号作用域与推荐有限并发；本机 Xcode 27 测试及 Release 构建结果见第 12 节。
...
## 12. 2026-09-21 增量实现与验证

### 实现范围

- `SubsonicClient` 使用可注入 `SubsonicTransport`，默认由 URLSession 执行请求；测试无需访问 NAS。
- 专辑分页默认每页 300 张，支持进度、取消、去重和重复分页保护。连接加载完成后进入音乐库；刷新失败不覆盖旧目录。
- 推荐曲目请求最多并发 4 个，并对输入专辑及输出歌曲去重，按专辑原顺序合并。保留当前视图的曲目缓存及持久化推荐快照；刷新失败不删除上一份推荐。
- 为保持本机推荐评分的完整候选集，本轮保留逐专辑聚合。随机候选会改变覆盖范围，因此暂未替换算法。并发减少等待轮数，不减少唯一专辑所需的请求数量。
- 账号切换会使旧的目录、收藏和推荐异步结果失效，避免写回新账号。

### 数据兼容与清除范围

- 账号键由规范化服务器地址与区分大小写的用户名计算 SHA-256 摘要，不包含密码、token 或完整鉴权 URL。
- 历史、行为和推荐继续使用现有容器，但在其中按账号键索引；恢复队列改用 `lastPlaybackState.v3.<账号键>` 独立保存。
- URL-only 历史、行为、推荐及旧队列保留在本机，不自动归属任何账号，也不会作为当前账号数据读取。升级后本机历史重新积累，旧队列不会自动恢复；服务端收藏和歌单不受影响。
- “更换服务器或账号”保留各账号记录。“清除本机播放记录”清除当前账号历史、行为和推荐；退出登录还清除当前账号队列和已保存连接。其他账号和没有归属的旧数据保持不变。

### 验证记录

- 环境：Xcode 27.0（27A266a）、iOS 27.0、iPhone 17 Simulator。
- 最终完整测试：5 个文件、29 项测试，0 failures，`TEST SUCCEEDED`。命令：`xcodebuild -project NaviLyrics.xcodeproj -scheme NaviLyrics -configuration Debug -destination 'platform=iOS Simulator,id=31920FC8-FA65-4E77-A6F9-CC4EEB5CAB84' -derivedDataPath build/ImprovementTests -parallel-testing-enabled NO -collect-test-diagnostics never CODE_SIGNING_ALLOWED=NO test`。
- 最终测试产物：`build/ImprovementTests/Logs/Test/Test-NaviLyrics-2026.09.21_20-35-18-+0800.xcresult`。上轮出现过模拟器诊断收集超时；最终命令关闭额外诊断收集，未关闭或跳过任何测试断言。
- `git diff --check` 通过。
- Release：`xcodebuild -project NaviLyrics.xcodeproj -scheme NaviLyrics -configuration Release -destination 'generic/platform=iOS' -derivedDataPath build/ImprovementRelease CODE_SIGNING_ALLOWED=NO build`，`BUILD SUCCEEDED`。
- 回归数据规模：分页 0、301、600、1,000 张专辑；推荐 100、300、1,000 张专辑。检查分页请求数、并发上限、去重、稳定顺序、失败与账号隔离，不将模拟响应耗时当作 NAS 性能。
- 尚未完成：远端 CI、真实 Navidrome 集成、真机 UI/后台播放验收、真实网络首屏耗时与内存峰值测量。因此 LIB-001、DAT-001、REC-001 保持待验证；REL-001 已获得本机绿色结果，但 CI 合入门禁仍待远端验证。
